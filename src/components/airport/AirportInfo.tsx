...
import { useAirportMeta } from '@/hooks/useAirport'
...
export default function AirportInfo({ services, iata = 'LHR' }: AirportInfoProps) {
  const { airport } = useAirportMeta(iata)
  const byIcon: Record<string, AirportService> = Object.fromEntries(services.map((s) => [s.icon, s]))

...
import { Sun, Cloud, CloudRain, CloudLightning, CloudSnow, CloudFog, Wind, Umbrella } from 'lucide-react'
...
const ICONS: Record<WeatherSnapshot['icon'], LucideIcon> = {
  sun: Sun,
  cloud: Cloud,
  rain: CloudRain,
...
export default function CityWeatherCard({ cities, selected, onSelect, baseTimezone, connectionState, lastUpdated }: CityWeatherCardProps) {
    ...
    <Card className="p-5 sm:p-6">
      ...
      <Tabs value={city.iata} onValueChange={onSelect} className="mt-4">
        <TabsList className="w-full">
          {cities.map((item) => (
            <TabsTrigger key={item.iata ?? item.city} value={item.iata ?? item.city} className="flex-1">
              {item.iata}
            </TabsTrigger>
          ))}
        </TabsList>

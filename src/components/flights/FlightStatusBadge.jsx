...
import { CheckCircle, PlaneTakeoff, DoorOpen, TriangleAlert, Clock, PlaneLanding, XCircle, Navigation2, HelpCircle } from 'lucide-react'
import { cn } from '@/lib/utils'

const styles = {
  scheduled: { bg: 'bg-accent', fg: 'text-muted-foreground', icon: Clock },
  on_time: { bg: 'bg-emerald-400/10', fg: 'text-emerald-300', icon: CheckCircle },
  ...
  gate_closing: { bg: 'bg-warning-light', fg: 'text-warning-dark', icon: TriangleAlert },
  ...
  unknown: { bg: 'bg-accent', fg: 'text-muted-foreground', icon: HelpCircle },
}

export default function FlightStatusBadge({ status, label, pulse = false }) {

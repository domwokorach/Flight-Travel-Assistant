...
import { Search, RotateCcw, Clock, Loader2, Plane, Hash, MapPin, Building2 } from 'lucide-react'
...
const SEARCH_BY_OPTIONS = [
  ...
  { value: 'Flight number', label: 'Flight number', icon: Hash },
  { value: 'City', label: 'City', icon: MapPin },
  { value: 'Airport', label: 'Airport', icon: Building2 },
]

function airlineCodeFor(term: string): string | null {
...
export default function FlightSearch({ onSearch, onClear, loading = false, isLive = false }: FlightSearchProps) {
      ...
      <form onSubmit={submit}>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
          <div className="flex flex-col gap-1.5">
            ...
            <Input id="search-date" type="date" value={date} onChange={(e) => setDate(e.target.value)} className="h-11 lg:w-[170px]" />
          </div>

          <OptionDropdown label="Search by" options={SEARCH_BY_OPTIONS} value={searchBy} onChange={setSearchBy} triggerClassName="lg:w-[170px]" />
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Select value={filter} onValueChange={setFilter}>
              <SelectTrigger size="sm" className="h-11 lg:w-[170px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="flex gap-2">
            <Button type="submit" disabled={loading} className="h-11 flex-1 lg:flex-none">
              {loading ? <Loader2 className="size-4 animate-spin" /> : <Search className="size-4" />}
              Search
            </Button>
            <Button type="button" variant="outline" size="icon" onClick={clear} aria-label="Clear search" className="size-11 rounded-xl">

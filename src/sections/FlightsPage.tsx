...
export default function FlightsPage({ countdown }: FlightsPageProps) {
  ...
  const [selectedCity, setSelectedCity] = React.useState<SelectedCity>('destination')
        ...
        <div className="flex flex-col gap-4">
          {weather.cities.length > 0 && (
            <CityWeatherCard
              ...
              onSelect={(iata: string) => setSelectedCity(iata === 'JFK' ? 'destination' : iata === 'AMS' ? 'connection' : 'departure')}

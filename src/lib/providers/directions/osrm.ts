...
/**
 * OSRM's public demo router (router.project-osrm.org) only serves the "driving" profile,
 * so driving routes come from a real routing engine while walking/cycling fall back to a
 * great-circle distance estimate at typical speeds. Both link out to Google Maps for
 * turn-by-turn "Get Directions", which needs no API key for a plain deep link.
 */
